...
export function parseFlags(
    ...
    const flagValues = new Map<string, ParsableType | undefined>(
        Array.from(flagDefinitions.entries()).map(([k, v]) => [
            k,
            v.defaultValue,
        ])
    );
    ...
    function parse(
        ...
        flagName: string,
        flagValue?: string,
        dontUseNextArg?: boolean
    ) {
        const flag = flagDefinitions.get(flagName);
        if (!flag)
            throw new CommandResultError(
                CommandResultUtils.failParseFlagUnknown(flagName)
            );
        ...
        if (flagValue) {
            ...
            if (parsed.value === undefined)
                throw new CommandResultError(
                    CommandResultUtils.failParseFlagInvalid(flag, flagValue),
                    parsed.message
                );
            ...
            if (flag.type === "boolean") {
                ...
                if (dontUseNextArg || index + 1 >= args.length)
                    throw new CommandResultError(
                        CommandResultUtils.failParseFlagInvalid(flag, "")
                    );
                const parsed = parseValue(flag, message, args[index + 1]);
                if (parsed.value === undefined)
                    throw new CommandResultError(
                        CommandResultUtils.failParseFlagInvalid(
                            flag,
                            args[index + 1]
                        ),
                        parsed.message
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (f.match(/^--[^-].*$/)) {
            ...
            if (!flagName)
                throw new CommandResultError(
                    CommandResultUtils.failParseFlagUnknown(parts[0])
                );
            ...
            for (let j = 0; j < flags.length; j++) {
                ...
                if (!flagName)
                    throw new CommandResultError(
                        CommandResultUtils.failParseFlagUnknown(flags[j])
                    );
